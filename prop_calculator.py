...
# --- ODDS API KEY ---
# Pulled from .streamlit/secrets.toml (which is gitignored, so it never gets
# committed). Falls back to a placeholder if the secret isn't set yet, so the
# app still loads and the sidebar box below lets you paste a key manually.
API_KEY = st.secrets.get("ODDS_API_KEY", "YOUR_THE_ODDS_API_KEY")
